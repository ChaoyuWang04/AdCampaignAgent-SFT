...
这个目录主要保存 benchmark 的文档与报告辅助脚本；真正的 benchmark 源码已经迁到 `src/benchmark/`，用于比较 base model、本地 SFT 模型、以及 OpenAI-compatible API 模型在 AdCampaignAgent 场景下的完整 trace 表现。
...
`src/benchmark/benchmark_runner.py` 里统一抽象为 `run_case(case) -> trace`。
