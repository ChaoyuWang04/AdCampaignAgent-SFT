...
- 原始样本：
  - `data/raw/ad_agent_seeds_<timestamp>_<lang>.json`
- 切分后的训练/测试集：
...
### `src/datapipeline/generate_base_dataset.py`

- 作用：规则生成 Ad Campaign Agent 原始 seed 样本。
- 运行方式：
  - `python src/datapipeline/generate_base_dataset.py`
- 输入：
...
- 输出文件形式：
  - JSON 数组。
- 默认输出文件名：
  - `ad_agent_seeds_<timestamp>_<lang>.json`
- 建议输出位置：
  - `data/raw/`
...
- 作用：将原始样本切分为训练集和测试集。
- 运行方式：
  - `python src/datapipeline/split_dataset.py [args]`
- 输入：
  - Ad Agent seed JSON。
- 输出：
  - 训练集 JSON
