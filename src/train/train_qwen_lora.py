...
核心开关：
  --only_last_assistant   只对最后一条 assistant 回复计算 loss
  （不加此参数）          对所有 assistant 回复计算 loss

4组实验对应：
  实验A: 非multiturn 数据 + all_assistant
  实验B: 非multiturn 数据 + --only_last_assistant
  实验C: multiturn 数据   + all_assistant
  实验D: multiturn 数据   + --only_last_assistant
"""
