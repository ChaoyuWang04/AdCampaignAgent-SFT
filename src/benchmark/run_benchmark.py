...
这个脚本是 `src/benchmark/` 目录下真正的主入口，负责把整套 benchmark 串起来执行。
...
- benchmark 主逻辑：`src.benchmark.benchmark_runner`
- benchmark schema：`src.benchmark.benchmark_schema`
...
from src.common.project_paths import data_dir, outputs_dir
from src.benchmark.benchmark_runner import (
    BenchmarkSuiteRunner,
    LocalHFCaseRunner,
    OpenAICaseRunner,
)
from src.benchmark.benchmark_schema import load_cases
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=data_dir() / "benchmark",
        help="Directory containing benchmark case JSON files",
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=outputs_dir() / "benchmark",
