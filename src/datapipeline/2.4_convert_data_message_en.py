...
class MockToolExecutor:
    ...
    def validate_creative_spec(self, **kw) -> str:
        if self.scene == "validate_fail_size":
            return self._ok({"valid": False, "errors": [{
                "field": "file_size",
                "message": "File size 58.3MB exceeds platform limit of 50MB",
                "platform_limit": "50MB", "actual": "58.3MB"}]})
        if self.scene == "validate_fail_format":
            return self._ok({"valid": False, "errors": [{
                "field": "resolution",
                "message": "Resolution 1280x960 does not meet requirements; must be 9:16 or 1:1",
    ...
    def upload_creative_asset(self, **kw) -> str:
        if self.scene == "upload_partial_fail":
            return self._ok({"asset_id": f"ASSET_{random.randint(1000,9999)}",
                             "status": "partial",
                             "warning": "Asset uploaded but review is pending; estimated review time is 12 hours"})
        return self._ok({"asset_id": f"ASSET_{random.randint(1000,9999)}",
                         "status": "uploaded",
    ...
    def get_campaign_metrics(self, **kw) -> str:
        ...
        if self.scene == "creative_fatigue":
            ...
            result["fatigue_signal"]  = "CTR has been declining for 3+ consecutive days; creative refresh recommended"
...
def main(input_path: str  = "ad_agent_seeds.json",
    ...
    print(f"\n✅ Generation complete: {len(records)} conversations, {failed} failed")
    print(f"📊 Average message turns: {sum(turn_lens)/max(len(turn_lens),1):.1f} turns")
